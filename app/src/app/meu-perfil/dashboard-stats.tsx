...
export default function DashboardStats({ profileComplete, professionalName }: DashboardStatsProps) {
  ...
  const [allStats, setAllStats] = useState<Record<string, PeriodStats> | null>(null);
  ...
  useEffect(() => {
    fetch("/api/me/stats")
      .then((r) => r.json())
      .then((data) => {
        if (data["7d"]) setAllStats(data);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const stats = allStats?.[period] ?? { views: 0, whatsapp: 0, calls: 0 };
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-50 rounded-xl p-4 text-center">
          ...
          {loading ? (
            <div className="h-8 bg-gray-200 rounded animate-pulse w-12 mx-auto" />
          ) : (
      ...
      {!loading && stats.views === 0 && stats.whatsapp === 0 && stats.calls === 0 && (
        <p className="text-[10px] text-gray-400 mt-3 text-center">
          As métricas aparecerão conforme seu perfil receber visitas.
        </p>
      )}
    </div>
  );
